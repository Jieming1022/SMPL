...
class ImageList(datasets.VisionDataset):
    ...
    def __init__(self, root: str, classes: List[str], data_list_file: str, dx: Optional[str] = None,
                 transform: Optional[Callable] = None, target_transform: Optional[Callable] = None):
        super().__init__(root, transform=transform, target_transform=target_transform)
        if dx is None:
            self.samples = self.parse_data_file(data_list_file)
        else:
            self.samples = self.parse_dr_data_file(data_list_file, dx)
    ...
    def __getitem__(self, index: int) -> Tuple[Any, int, Any]:
        """
        Args:
            index (int): Index
    ...
    def parse_dr_data_file(self, file_name: str, dx: str) -> List[Tuple[str, int]]:
        """Parse file to data list for diabetic retinopathy datasets

        Args:
            file_name (str): The path of data file
            dx (str): The diagnosis type
            return (list): List of (image path, class_index) tuples
        """
        with open(file_name, "r") as f:
            data_list = []
            for line in f.readlines():
                ...
                if not os.path.isabs(path):
                    path = os.path.join(self.root, path)
                if dx == 'RDR':
                    target = 0 if int(target) < 2 else 1
                elif dx == 'NORM':
